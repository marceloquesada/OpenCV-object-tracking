...
"""
def get_e(ef, ei, deltap, pmax):
    e = ((ef - ei)/(pmax**2))*((deltap - pmax)**2) + ei
    print(str(e), "deltap:", str(deltap))
    return e
"""
def get_e(ef, ei, deltap, pmax):
    ei = 3.5*(ei) - 2.7*(ef)
    delta_e = ef - ei
    deltap = abs(0.5*deltap)
    e = ((delta_e/(pmax**2))*(deltap**2)) - ((delta_e/pmax)*deltap) + ef
    return e
...
def servo_communication(new_points, old_points):
    if abs(new_points[0] - old_points[0]) > POINTS_THRESHOLD and abs(new_points[1] - old_points[1]) > POINTS_THRESHOLD and status == 1:
        ...
        if abs(pos_to_center[0]) > POS_TO_CENTER_THRESHOLD or abs(pos_to_center[1]) > POS_TO_CENTER_THRESHOLD:
            ...
            e_x = get_e(E_SLOW, E_FAST, x_to_center, width/2)
            e_y = get_e(E_SLOW, E_FAST, y_to_center, height/2)
